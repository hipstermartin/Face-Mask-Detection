...
def mask_video():
    ...
    out = cv2.VideoWriter('./images/out_video_result.mp4', fourcc,
                          cap.get(cv2.CAP_PROP_FPS), (img.shape[1], img.shape[0]))
    ...
    while cap.isOpened():
        ...
        blob = cv2.dnn.blobFromImage(
            img, scalefactor=1., size=(300, 300), mean=(104., 177., 123.))
        ...
        for i in range(dets.shape[2]):
            ...
            cv2.rectangle(result_img, pt1=(x1, y1), pt2=(x2, y2),
...
def mask_detection():
    ...
    choice = st.sidebar.selectbox(
        "Select the type you want to detect", activities)

    if choice == 'Image':
        mask_image()
        st.subheader("Detection on Image")
        image_file = st.file_uploader(
            "Upload Image", type=['jpg'])  # upload image
        if image_file is not None:
            our_image = Image.open(image_file)  # making compatible to PIL
            im = our_image.save('./images/out.jpg')
            saved_image = st.image(
                image_file, caption='image uploaded successfully', use_column_width=True)
