...
def mask_stream():
    ...
    activities = ["Webcam" ,"Image"] #Button creation order
    choice = st.sidebar.selectbox("Select the type you want to detect",activities) #Button creation

    halfmask_sound = open('./sound/halfmask_sound.mp3','rb').read()
    nomask_sound = open('./sound/nomask_sound.mp3','rb').read()

    if choice=="Image": #Action when clicking the image button
        ...
        if image_file is not None:
            ...
            if st.button("Process"):
                ...
                if label == 0:
                    st.write("You are wearing mask well.")
                elif label == 1:
                    st.write("Please wear mask properly.")
